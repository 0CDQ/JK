...
try:
    ...
    data['欧洲汇总行'].fillna('', inplace = True)
    title_list = ['欧洲汇总行',
                  'MSKU',
                  '店铺',
                  '国家',
    ...
    # print(data)
    # 清洗欧洲站数据
    if '共享库存' in data['欧洲汇总行'].values:
        data = data.drop(data[(data['欧洲汇总行'] == '共享库存')].index)
        data = data.reset_index(drop = True)
        # eu_store = data['店铺'].iloc[data[(data['欧洲汇总行'] == '是')].index].apply(lambda x: x.split('-')[0]).tolist()
        data.loc[data['欧洲汇总行'] == '是', '店铺'] = data['店铺'].iloc[data[(data['欧洲汇总行'] == '是')].index].apply(lambda x: x.split('-')[0])
    # 清洗美国站数据
    if '美国' in data['国家'].values:
        data = data.drop(data[(data['国家'] == '墨西哥') | (data['国家'] == '加拿大')].index)
        data = data.reset_index(drop = True)
        data.loc[(data['国家'] == '美国'), '店铺'] = data['店铺'].iloc[data[(data['国家'] == '美国')].index].apply(
            lambda x: x.split('-')[0])
    ...
    data = pd.merge(data, df_useless, how='left', on=['店铺', 'MSKU'])
    data.loc[(data['备货天数淘汰'] == 0), '备货天数'] = 0
    data['备货天数'].fillna('', inplace=True)
